...
public class MaxHeap <T extends Comparable<? super T>>{
    ...
    public MaxHeap(Object[] arr, int size){
        this.size = size;
    ...
    public void insert(T value){
        ...
        this.arr[size] = value;
        //now we must check if this is added correctly by asking its parent "am I biggger then you?"
    ...
    private void moveDown(int baseIndex){
        ...
        int maxChildIndex = maxChildIndex(baseIndex);

        while(maxChildIndex != -1) {
            T maxChild = (T)this.arr[maxChildIndex];
            if(base.compareTo(maxChild) >= 0) break;
            this.arr[baseIndex] = maxChild;
            this.arr[maxChildIndex] = base;
            baseIndex = maxChildIndex;
            maxChildIndex = maxChildIndex(baseIndex);
        }
    }
    ...
    public static <T extends Comparable<? super T>> MaxHeap<T> heapify(T[] input){
        ...
        MaxHeap<T> heap = new MaxHeap<>(copiedArr, input.length);
